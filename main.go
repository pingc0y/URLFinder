...
func usage() {
	fmt.Fprintf(os.Stderr, `URLFinder 2022/6/21  by pingc
...
func main() {
	...
	if f!=""{
		...
		for {
			...
			if s {fmt.Println("Start Validate...")}
...
func outFile(){
	...
	for _,j :=range resultJsHost{
		if len(j)>10 && (strings.Contains(j," | ") || !s){
	...
	for _,u :=range resultUrlHost{
		if len(u)>10 && (strings.Contains(u," | ") || !s){
...
func jsFind(cont ,host,scheme,path string,is bool)  {
	...
	for _,re:= range res {
		...
		//循环提取js放到结果中
		for _, js := range jss {
			if strings.HasPrefix(js[0], "http") {
				appendJs(js[0])
				if is || m == 2{
					wg.Add(1)
...
func urlFind(cont ,host,scheme,path string,is bool) {
	...
	for _,re:= range res {
		...
		for _, url := range urls {
			...
			if strings.HasPrefix(url[0], "http") {
				appendUrl(url[0])
				if is && m==2{
					wg.Add(1)
